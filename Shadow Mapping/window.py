...
class Window:
    ...
    def tick(self):
        ...
        sdl2.SDL_Delay(int(1000/self.fpsTarget))
        self._fpsDisplayCounter += diff
        if self._fpsDisplayCounter >= self._fpsDisplayDelay:

            sdl2.SDL_SetWindowTitle(self.screen, b'OpenGL test')
            # fps = str(self.fps)
            # titile = self.title
            # str = titile + ": " + fps
            # sdl2.SDL_SetWindowTitle(self.screen, str)
