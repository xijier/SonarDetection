...
if __name__ == "__main__":
    ...
    for imageName in files_img_obj:
        ...
        for index, item in enumerate(croppedList):
            ...
            # cv2.imshow('image', shrink_target)
            # cv2.waitKey(0)
            matRotate = cv2.getRotationMatrix2D((h_0 * 0.5, w_0 * 0.5), angle,1)  # mat rotate 1 center 2 angle 3 缩放系数
            shrink_target = cv2.warpAffine(shrink_target, matRotate, (h_0, w_0))
        ...
        imageName = imageName.split(".")[0] + "_"+ str(shrink_size[0])+"_"+str(shrink_size[1])+"_Angle_" +str(angle) + ".jpg"
