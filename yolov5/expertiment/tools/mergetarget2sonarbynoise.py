...
def main(shape_noise,noise_size):

    base = 'target_'
    foler = base + shape_noise + "_" + noise_size

    path = os.path.abspath('.')

    img_folder_path = "E:\kg\data\SONAR_VOC_MulitScale_styletransfer\VOC2007_Style_1\content_styled/"
    target_img_path = "E:\kg\data\SONAR_VOC_MulitScale_styletransfer\VOC2007_Style_1/"+foler+"/JPEGImages/"
    target_xml_path = "E:\kg\data\SONAR_VOC_MulitScale_styletransfer\VOC2007_Style_1/"+foler+"/Annotations/"
    JPEGImages_pool = "E:\kg\data\SONARIMG_pool/"
    # Style_background_image = "E:\kg\yolov5-master\expertiment\data\style_pool/style_2.jpg"
    # Style_background_image_1 = "E:\kg\yolov5-master\expertiment\data\style_pool/style_4_1.jpg"
    lables = ['aeroplane', 'bicycle', 'car', 'person', 'ship']
    #shape_noise = ['origin', 'circle', 'line', 'rectangle']
    #noise_size = ['4', '6', '8']
    # 4种缩放类别
    ...
    for index_img in range(0, 11000):
        ...
        for label in target_labels:
            ...
            if random.random() < 0.5:
                shape_noise_label = shape_noise
                image_path = img_folder_path + label + "/" + shape_noise_label + "/" + noise_size
            else:
                shape_noise_label = "origin"
                image_path = img_folder_path + label + "/" + shape_noise_label + "/"
        ...
        #     img_sonar = cv2.imread(Style_background_image_1)
        # else:
...
def main_combine(noise_size):

    base = 'target_combine_'
    foler = base + noise_size
    ...
    for index_img in range(0, 11000):
        ...
        for label in target_labels:
            ...
            p = random.random()
            if p < 0.25:
                shape_noise_label = "circle"
                image_path = img_folder_path + label + "/" + shape_noise_label + "/" + noise_size
            elif p > 0.25 and p < 0.5:
                shape_noise_label = "line"
                image_path = img_folder_path + label + "/" + shape_noise_label + "/" + noise_size
            elif p > 0.5 and p < 0.75:
                shape_noise_label = "rectangle"
                image_path = img_folder_path + label + "/" + shape_noise_label + "/" + noise_size
            else:
                shape_noise_label = "origin"
                image_path = img_folder_path + label + "/" + shape_noise_label + "/"
...
if __name__ == "__main__":
    ...
    # folders = ['target_circle_4', 'target_circle_6', 'target_circle_8',
    #           'target_line_4', 'target_line_6', 'target_line_8',
    #           'target_rectangle_4', 'target_rectangle_6', 'target_rectangle_8']
    # shape_noises = ['circle', 'line', 'rectangle']
    # noise_sizes = ['4', '6', '8']
    #
    # base = 'target_'
    # for shape_noise in shape_noises:
    #     for noise_size in noise_sizes:
    #         main(shape_noise,noise_size)
    #         print(base+shape_noise+"_"+noise_size)

    folders = ['target_combine_4', 'target_combine_6', 'target_combine_8']
    noise_sizes = ['4', '6', '8']
    for noise_size in noise_sizes:
        main_combine(noise_size)
